...
class Preprocessor:
    ...
    def analyze_style(self, text):
        ...
        passive_sentences = sum(
            1 for sent in doc.sents if self.is_passive(sent))
    ...
    def analyze_vocabulary(self, text):
        ...
        doc = self.nlp(text)
        unique_words = set(token.text.lower()
    ...
    def extract_key_terms(self, text):
        ...
        key_terms_list = ['segurança', 'risco', 'vulnerabilidade',
                          'ataque', 'criptografia', 'firewall', 'cyber']

        doc = self.nlp(text)
        document_words = set([token.text.lower() for token in doc])

        key_terms = set(
            term for term in key_terms_list if term in document_words)
    ...
    def analyze_topics(self, text):
        ...
        sorted_words = sorted(
            word_freq.items(), key=lambda kv: kv[1], reverse=True)
